...
class Glm5FlashTinyConfig(KimiLinearConfig):
    ...
    def __init__(
        ...
        # DSA's FP8 MLA cache stores the non-rotary key slice in a fixed
        # 512-wide tile. Keep that cache-facing width while scaling heads,
        # layers, and latent ranks for the tiny model.
        qk_nope_head_dim: int = 512,
        # Keep a non-zero rotary slice because SGLang's DSA backend uses it
        # for the indexer/attention cache even in this reduced configuration.
