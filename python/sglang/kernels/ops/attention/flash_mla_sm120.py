...
_GLM_DSA_MODEL_ARCHS = (
    ...
    # The tiny adapter keeps the same FP8 DSA cache contract with reduced
    # heads/top-k, so it is allowed to use this SM120 dispatch family too.
...
def flashinfer_sparse_mla_forward(
    ...
    *,
    ...
    topk = indices.shape[1]
    # FlashInfer's SM120 v32 table still hard-codes the production GLM
    # query shape (512 latent + 64 rotary).  The tiny adapter intentionally
    # removes that rotary tail; dispatch directly to the packed-cache eager
    # implementation instead of asking FlashInfer to manufacture one.
    if (
        qk_rope_head_dim == 0
        and qk_nope_head_dim == kv_lora_rank == 512
        and topk != 2048
    ):
    ...
    try:
        result = trtllm_batch_decode_with_kv_cache_mla(
            ...
            # NoPE MLA cannot infer the active sparse length from a rotary
            # tail; pass the per-row DSA lengths explicitly.
            sparse_mla_top_k_lens=seq_lens,
        )
        return result.squeeze(1)
    except ValueError as exc:
        # FlashInfer's dispatch table intentionally covers production GLM
        # shapes.  Tiny/reference shapes retain the same packed cache
        # semantics but use a smaller top-k/head count, so keep a readable
        # eager fallback for those unsupported combinations.
...
def _torch_sparse_mla_forward(
    *,
    ...
    bf16 rotary KV]``.  Decode that row format directly, gather selected
    ...
    output = torch.einsum(
        "thk,tkv->thv", weights, selected[..., :kv_lora_rank].float()
    )
